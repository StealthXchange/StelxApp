...
export default function UnshieldPage() {
  ...
  const valid = to.trim() ? isAddress(to.trim()) : null;
  ...
  return (
    <div className="pane">
      <h1 className="pane-title">Withdraw</h1>
      <p className="hint warn">The amount and destination are public.</p>
      ...
      <div className="card pane-card">
        ...
        {parsed !== null && parsed > 0n && (
          <div className="fee-row mono">
            ...
            <span>{formatAmount(parsed - (parsed * FEE_BPS) / BPS, asset, multiplier)} {asset.symbol}</span>
          </div>
        )}

        {state.stage === "idle" && (
          <button className="btn" disabled={!canSend} onClick={() => build("unshield", asset.address, to.trim(), parsed!, broadcaster!.address, broadcaster!.shieldedAddress, fee)}>
            Withdraw
          </button>
        )}
      </div>
